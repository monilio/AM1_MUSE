...
% Comienza el segundo de los hitos en MATLAB, el poder que este lenguaje
% posee ya ha sido demostrado a los revolucionarios. Estos posiblemente
% planean un golpe de vuelta. Estare preparado (a menos que me aburra de esto)
...
function Euler_method(U, N, dt, t0)
...
function CrankNicolson_method(U, N, dt, t0)
...
function CrankNicolsonMejorado_method(U, N, dt, t0, iter)
...
function CrankNicolsonNewton_method(U, N, dt, t0, max_iter, tol)
    ...
    for i = 1:N
        ...
        for k = 1:max_iter
            ...
            JF = Jacobiano(@(Z) F(Z, t+dt), U_pred);
            JG  = eye(4) - (dt/2) * JF;                         % Jacobiano de G

            delta = - JG \ G(:);                                % Paso de Newton: JG * delta = -G
                                                                % Esto es resolver el sistema, no dividir
...
function RungeKutta4_method(U, N, dt, t0)
...
function EvilEuler_method(U, N, dt, t0)
    ...
    graficar(posiciones, velocidades, "Euler Inverso");      % Se llama a la funcion de graficar
end




function EvilEulerMejorado_method(U, N, dt, t0, iter)
    ...
    graficar(posiciones, velocidades, "Euler Inverso Mejorado");      % Se llama a la funcion de graficar
end
